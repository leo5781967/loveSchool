				...
				<div class="w3-padding-64 w3-content w3-text-grey" id="contact">
					<div id="page-wrapper">
						<!-- Banner -->
							<section id="banner">
								<header>
										<h2 style='color:#4B0091;'>愛校紀錄統計-班級</h2>
										<h2 style='color:#4B0091;'><?php echo $_GET["seme"].$_GET["stu"].$_GET["grade"]."年  "; ?></h2>
								</header>
						
									<?php
											$select="SELECT e.`class_name`,count(DISTINCT a.`account_id`) as `totalpeople`,sum(b.`res_hr`) as `totalhr` FROM `master_list` as a inner join `reservation` as b on a.`mas_id`=b.`mas_id` inner join `student_information` AS e ON a.`account_id` = e.`account_id` where b.`res_status`='1' and a.`semester`='$_GET[seme]' and e.`stu_dep`='$_GET[stu]' and e.`stu_grade`='$_GET[grade]' group by e.`class_name`";
											
											//合計
											$select1="SELECT count(DISTINCT a.`account_id`) as `totalpeople1`,sum(b.`res_hr`) as `totalhr1` FROM `master_list` as a inner join `reservation` as b on a.`mas_id`=b.`mas_id` inner join `student_information` AS e ON a.`account_id` = e.`account_id` where b.`res_status`='1' and a.`semester`='$_GET[seme]' and e.`stu_dep`='$_GET[stu]' and e.`stu_grade`='$_GET[grade]'";
											...
											$ret1=mysqli_query($link,$select1);
												...
												while($row=mysqli_fetch_assoc($ret)){
													...
													"<tr>
														<td align='center'>
														<a href='M-Option_4.php?seme=$_GET[seme] & stu=$_GET[stu] & grade=$_GET[grade] & class=$row[class_name]'>
														$row[class_name]
														</a>
														</td>
