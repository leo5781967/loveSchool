			...
			<div class="w3-padding-large" id="main">
				<div class="w3-padding-64 w3-content w3-text-grey" id="contact">
					<div id="page-wrapper">
							<section id="banner">
								<header>
										<h2 style='color:#4B0091;'>事由統計-其他事由-科系年級班級</h2>
										<h2 style='color:#4B0091;'><?php echo $_GET["seme"]." 其他 ".$_GET["con"].$_GET["stu"].$_GET["grade"]."年"; ?></h2>
								</header>
									<?php
											$count="SELECT e.`class_name`,count(a.`account_id`) as `totalpeople` FROM `reservation` as a1 inner join `master_list` as a on a1.`mas_id`=a.`mas_id` INNER JOIN `student_information` AS e ON a.`account_id` = e.`account_id` where a1.`res_status`='1' and a.`content_number`='1' and a.`semester`='$_GET[seme]' and a.`content`='$_GET[con]' and e.`stu_dep`='$_GET[stu]' and e.`stu_grade`='$_GET[grade]' GROUP BY e.`class_name`";
											
											//合計
											$select1="SELECT count(a.`account_id`) as `totalpeople1`,sum(b.`res_hr`) as `totalhr1` FROM `master_list` as a inner join `reservation` as b on a.`mas_id`=b.`mas_id` inner join `student_information` AS e ON a.`account_id` = e.`account_id` where b.`res_status`='1' and `content_number`='1' and a.`semester`='$_GET[seme]' and a.`content`='$_GET[con]' and e.`stu_dep`='$_GET[stu]' and e.`stu_grade`='$_GET[grade]'";
											...
											while($row=mysqli_fetch_assoc($count1)){
												$select1="SELECT e.`class_name`,count(DISTINCT a.`account_id`) as `people`,sum(b.`res_hr`) AS `totalhr` FROM `master_list` AS a INNER JOIN `reservation` AS b ON a.`mas_id` = b.`mas_id` INNER JOIN `units` AS d ON b.`unit_id` = d.`unit_id` inner join `student_information` as e on a.`account_id`=e.`account_id` where b.`res_status`='1' and a.`content_number`='1' and a.`semester`='$_GET[seme]' and a.`content`='$_GET[con]' and e.`stu_dep`='$_GET[stu]' and e.`stu_grade`='$_GET[grade]' and e.`class_name`='$row[class_name]'";
												$ret1=mysqli_query($link,$select1);
												while($row1=mysqli_fetch_assoc($ret1)){
													echo
													"<tr>
														<td align='center'>
														<a href='statistics_4_1_5.php?seme=$_GET[seme] & con=$_GET[con] & stu=$_GET[stu] & grade=$_GET[grade] & class=$row1[class_name]'>
														$row1[class_name]
														</a>
														</td>
