				...
				<div class="w3-padding-64 w3-content w3-text-grey" id="contact">
					<div id="page-wrapper">
						<!-- Banner -->
							<section id="banner">
								<header>
										<h2 style='color:#ffffff;'>統計各處室(科)-班級</h2>
										<h2 style='color:#ffffff;'><?php echo $_GET["seme"].$_GET["unitname"].$_GET["stu"].$_GET["grade"]."年"; ?></h2>
								</header>
						
									<?php
											$select="SELECT class_name,count(DISTINCT e.`account_id`) as `totalpeople`,sum(c.`hr`) as `totalhr` FROM `master_list` AS a,`reservation` AS b, `service_hours` AS c,`units` AS d,`student_information` as e where b.`res_status`='1' and a.`mas_id` = b.`mas_id` AND b.`res_id` = c.`res_id` AND b.`unit_id` = d.`unit_id` and a.`account_id`=e.`account_id` and a.`semester`='$_GET[seme]' and d.`unit_name`='$_GET[unitname]' and e.`stu_dep`='$_GET[stu]' and e.`stu_grade`='$_GET[grade]' group by e.`class_name`";
											$ret=mysqli_query($link,$select);
												...
												while($row=mysqli_fetch_assoc($ret)){
													...
													"<tr>
														<td align='center'>
														<a href='t_statistics_s4.php?seme=$_GET[seme] & unitname=$_GET[unitname] & stu=$_GET[stu] & grade=$_GET[grade] & class=$row[class_name]'>
														$row[class_name]
														</a>
														</td>
