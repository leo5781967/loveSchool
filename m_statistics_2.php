			...
			<div class="w3-padding-large" id="main">
				<div class="w3-padding-64 w3-content w3-text-grey" id="contact">
					<div id="page-wrapper">
							<section id="banner">
								<header>
									<h2 style='color:#ffffff;'>本班事由統計-其他事由</h2>
									<h2 style='color:#ffffff;'><?php echo $_GET["seme"]." 其他 "; ?></h2>
								</header>
									<?php
											$count="SELECT a.`content`,count(a.`account_id`) as `totalpeople` FROM `reservation` as a1 inner join `master_list` AS a on a1.`mas_id`=a.`mas_id` INNER JOIN `student_information` AS e ON a.`account_id` = e.`account_id` where a1.`res_status`='1' and a.`content_number`='1' and e.`teacher_id` = '$tt' and a.`semester`='$_GET[seme]' GROUP BY a.`content`";
											
											//合計
											$select1="SELECT count(a.`account_id`) as `totalpeople1`,sum(b.`res_hr`) as `totalhr1` FROM `master_list` as a inner join `reservation` as b on a.`mas_id`=b.`mas_id` inner join `student_information` AS e ON a.`account_id` = e.`account_id` where b.`res_status`='1' and a.`content_number`='1' and e.`teacher_id` = '$tt' and a.`semester`='$_GET[seme]'";
											...
											while($row=mysqli_fetch_assoc($count1)){
												$select1="SELECT a.`content`,count(DISTINCT a.`account_id`) as `people`,sum(b.`res_hr`) as `totalhr` FROM `master_list` as a inner join `reservation` as b on a.`mas_id`=b.`mas_id` inner join `units` as d on b.`unit_id`=d.`unit_id` inner join `student_information` as e on a.`account_id`=e.`account_id` inner join `teacher_information` as f on a.`teacher_id`=f.`teacher_id` where b.`res_status`='1' and a.`content_number`='1' and e.`teacher_id` = '$tt' and a.`semester`='$_GET[seme]' and a.`content`='$row[content]'";
												$ret1=mysqli_query($link,$select1);
												while($row1=mysqli_fetch_assoc($ret1)){
													echo
													"<tr>
														<td align='center'>													
														<a href='student_inf.php?seme=$_GET[seme] & con=$row1[content]'>
														$row1[content]</a>
														</td>
