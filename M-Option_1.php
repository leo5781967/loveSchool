			...
			<div class="w3-padding-large" id="main">
				<div class="w3-padding-64 w3-content w3-text-grey" id="contact">
					<div id="page-wrapper">
							<section id="banner">
								<header>
										<h2 style='color:#4B0091;'>愛校紀錄統計-科系</h2>
										<h2 style='color:#4B0091;'><?php echo $_GET["seme"]; ?></h2>
								</header>
									<?php
											$select="SELECT e.`stu_dep`,count(DISTINCT a.`account_id`) as `totalpeople`,sum(b.`res_hr`) as `totalhr` FROM `master_list` as a inner join `reservation` as b on a.`mas_id`=b.`mas_id` inner join `student_information` AS e ON a.`account_id` = e.`account_id` where b.`res_status`='1' and a.`semester`='$_GET[seme]' group by e.`stu_dep`";
											
											//合計
											$select1="SELECT count(DISTINCT a.`account_id`) as `totalpeople1`,sum(b.`res_hr`) as `totalhr1` FROM `master_list` as a inner join `reservation` as b on a.`mas_id`=b.`mas_id` where b.`res_status`='1' and a.`semester`='$_GET[seme]'";
											...
											$ret1=mysqli_query($link,$select1);
												...
												while($row=mysqli_fetch_assoc($ret)){
													...
													"<tr>
														<td align='center'>
														<a href='M-Option_2.php?seme=$_GET[seme] & stu=$row[stu_dep]'>
														$row[stu_dep]
														</a>
														</td>
