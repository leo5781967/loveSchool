		...
		<div class="w3-padding-64 w3-content w3-text-grey" id="contact">
			<div id="page-wrapper">
				<section id="banner">
					...
					?>
							<h2 style='color:#ffffff;'>本班事由統計人數人次時數<br/>學期：<?php echo "$_POST[year]";?></h2>
					</header>
						
					<?php
							$year=$_POST['year'];
							//============計算是否有資料============
							$select_sql2="SELECT count(a.`account_id`) as `totalpeople` FROM `master_list` as a inner join `reservation` as b on a.`mas_id`=b.`mas_id` inner join `service_hours` as c on b.`res_id`=c.`res_id` inner join `units` as d on b.`unit_id`=d.`unit_id` inner join `student_information` as e on a.`account_id`=e.`account_id` inner join `teacher_information` as f on a.`teacher_id`=f.`teacher_id` where b.`res_status`='1' and a.`content_number`='0' and e.`teacher_id` = '$tt'";
							
							$select1_sql2="SELECT count(a.`account_id`) as `totalpeople` FROM `master_list` as a inner join `reservation` as b on a.`mas_id`=b.`mas_id` inner join `service_hours` as c on b.`res_id`=c.`res_id` inner join `units` as d on b.`unit_id`=d.`unit_id` inner join `student_information` as e on a.`account_id`=e.`account_id` inner join `teacher_information` as f on a.`teacher_id`=f.`teacher_id` where b.`res_status`='1' and a.`content_number`='1' and e.`teacher_id` = '$tt'";
							//======================================
							
							//============統計人次==================
							$count="SELECT a.`semester`,a.`content`,count(a.`account_id`) as `totalpeople` FROM `reservation` as a1 inner join `master_list` as a on a1.`mas_id`=a.`mas_id` INNER JOIN `student_information` AS e ON a.`account_id` = e.`account_id` where a1.`res_status`='1' and a.`content_number`='0' and e.`teacher_id` = '$tt'";
											
							$count1="SELECT a.`semester`,a.`content`,count(a.`account_id`) as `totalpeople` FROM `reservation` as a1 inner join `master_list` as a on a1.`mas_id`=a.`mas_id` INNER JOIN `student_information` AS e ON a.`account_id` = e.`account_id` where a1.`res_status`='1' and a.`content_number`='1' and e.`teacher_id` = '$tt'";
							//======================================
							
							//其他人數----------------------------------
							$totalnum=0;
							$count2="SELECT a.`content`,count(a.`account_id`) as `totalpeople` FROM `reservation` as a1 inner join `master_list` as a on a1.`mas_id`=a.`mas_id` INNER JOIN `student_information` AS e ON a.`account_id` = e.`account_id`  where a1.`res_status`='1' and a.`content_number`='1' and a.`semester`='$year' GROUP BY a.`content`";
							
							$count3=mysqli_query($link,$count2);
							
							while($row=mysqli_fetch_assoc($count3)){
								$select3="SELECT a.`content`,count(DISTINCT a.`account_id`) as `people`,sum(b.`res_hr`) AS `totalhr` FROM `master_list` AS a INNER JOIN `reservation` AS b ON a.`mas_id` = b.`mas_id` INNER JOIN `units` AS d ON b.`unit_id` = d.`unit_id` INNER JOIN `student_information` AS e ON a.`account_id` = e.`account_id` where b.`res_status`='1' and a.`content_number`='1' and a.`semester`='$year' and e.`teacher_id` = '$tt' and a.`content`='$row[content]'";
								$ret1=mysqli_query($link,$select3);
							...
							//合計
							$select2="SELECT a.`semester`,count(a.`account_id`) as `totalpeople1`,sum(b.`res_hr`) as `totalhr1` FROM `master_list` as a inner join `reservation` as b on a.`mas_id`=b.`mas_id` inner join `student_information` as e on a.`account_id`=e.`account_id` where b.`res_status`='1' and e.`teacher_id` = '$tt'";
							...
							while($roww=mysqli_fetch_assoc($count11)){					
								...
								while($select_row=mysqli_fetch_array($select)){
									echo "
										<tr>
											<td align='center'>
											<a href='student_inf1.php?seme=$year & con=$select_row[content]'>
												$select_row[content]
											</a>
											</td>
											<td align='center'>$select_row[people]</td>
											<td align='center'>$roww[totalpeople]</td>
											<td align='center'>$select_row[totalhr]</td>
										</tr>";
								$totalnum1+=$select_row["people"];	
								}
							}
							$totalnum2=0;
							while($rowww=mysqli_fetch_assoc($count2)){	
								...
								//分其他
								while($select_row1=mysqli_fetch_array($select1)){
								//等於0消失				
									if($totalnum != 0){
										echo "
											<tr>
												<td align='center'>
													<a href='m_statistics_2.php?seme=$year'>其他</a>
												</td>
												<td align='center'>$totalnum</td>
												<td align='center'>$rowww[totalpeople]</td>
												<td align='center'>$select_row1[totalhr]</td>
											</tr>";
									}
					...
					<div name="back" align="center">
						<br/>
						<input type="button" id="back" name="back" class="red" onclick="location.href='myclass_search.php'" value="返回首頁" />
					</div>
				</section>
				<br/>			
			</div>
