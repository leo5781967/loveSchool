						...
						<!-- Banner -->
							<section id="banner">
								<header>
										...
										<h2 style='color:#ffffff;'><?php echo $_GET["seme"].$_GET["con"]; ?></h2>
								</header>
						
									<?php	
											$count="SELECT e.`account_id`,e.`stu_name`,count(a.`account_id`) as `totalpeople` FROM `reservation` as a1 inner join `master_list` as a on a1.`mas_id`=a.`mas_id` INNER JOIN `student_information` AS e ON a.`account_id` = e.`account_id`  where a1.`res_status`='1' and a.`content_number`='0' and e.`teacher_id` = '$tt' and a.`semester`='$_GET[seme]' and a.`content`='$_GET[con]' GROUP BY e.`account_id`";
											
											//合計
											$select2="SELECT a.`semester`,count(a.`account_id`) as `totalpeople1`,sum(b.`res_hr`) as `totalhr1` FROM `master_list` as a inner join `reservation` as b on a.`mas_id`=b.`mas_id` inner join `student_information` as e on a.`account_id`=e.`account_id` where b.`res_status`='1' and a.`content_number`='0' and e.`teacher_id` = '$tt' and a.`semester`='$_GET[seme]' and a.`content`='$_GET[con]'";
											...
											while($row=mysqli_fetch_assoc($count1)){
												$select1="SELECT e.`account_id`,e.`stu_name`,sum(b.`res_hr`) as `totalhr` FROM `master_list` as a inner join `reservation` as b on a.`mas_id`=b.`mas_id` inner join `units` as d on b.`unit_id`=d.`unit_id` inner join `student_information` as e on a.`account_id`=e.`account_id` inner join `teacher_information` as f on a.`teacher_id`=f.`teacher_id`  where `content_number`='0' and e.`teacher_id` = '$tt' and a.`semester`='$_GET[seme]' and a.`content`='$_GET[con]' and e.`account_id`='$row[account_id]'";
												...
												while($row1=mysqli_fetch_assoc($ret1)){
													...
													"<tr>
														...
														<td align='center'>
														<a href='student_inf1_1.php?seme=$_GET[seme] & con=$_GET[con] & name=$row1[stu_name]''>
														$row1[totalhr]
														</a>
														</td>
													</tr>";
