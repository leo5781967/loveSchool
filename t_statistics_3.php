				...
				<div class="w3-padding-64 w3-content w3-text-grey" id="contact">
					<div id="page-wrapper">
						<!-- Banner -->
							<section id="banner">
								<header>
										<h2 style='color:#ffffff;'>本科事由統計-班級</h2>
										<h2 style='color:#ffffff;'><?php echo $_GET["seme"].$_GET["con"]."  ".$_SESSION['sps_dep']."  ".$_GET["grade"]."年"; ?>
								</header>
						
									<?php
									
											$count="SELECT e.`class_name`,count(a.`account_id`) as `totalpeople` FROM `reservation` as a1 inner join `master_list` AS a on a1.`mas_id`=a.`mas_id` INNER JOIN `student_information` AS e ON a.`account_id` = e.`account_id` where a1.`res_status`='1' and a.`content_number`='0' and e.`stu_dep` LIKE '%$tt%' and a.`semester`='$_GET[seme]' and a.`content`='$_GET[con]' and e.`stu_grade`='$_GET[grade]' group by e.`class_name`";
											
											//合計
											$selectt="SELECT count(a.`account_id`) as `totalpeople1`,sum(b.`res_hr`) as `totalhr1` FROM `master_list` as a inner join `reservation` as b on a.`mas_id`=b.`mas_id` inner join units as d on b.`unit_id` = d.`unit_id` inner join `student_information` AS e ON a.`account_id` = e.`account_id` where b.`res_status`='1' and a.`content_number`='0' and e.`stu_dep` LIKE '%$tt%' and a.`semester`='$_GET[seme]' and a.`content`='$_GET[con]' and e.`stu_grade`='$_GET[grade]'";
											...
											while($row1=mysqli_fetch_assoc($count1)){
												$select="SELECT e.`class_name`,count(DISTINCT a.`account_id`) as `people`,sum(b.`res_hr`) as `totalhr` FROM `master_list` as a inner join `reservation` as b on a.`mas_id`=b.`mas_id` inner join `units` as d on b.`unit_id`=d.`unit_id` inner join `student_information` as e on a.`account_id`=e.`account_id` inner join `teacher_information` as f on a.`teacher_id`=f.`teacher_id` where b.`res_status`='1' and a.`content_number`='0' and e.`stu_dep` LIKE '%$tt%' and a.`semester`='$_GET[seme]' and a.`content`='$_GET[con]' and e.`stu_grade`='$_GET[grade]' and e.`class_name`='$row1[class_name]'";
												$ret=mysqli_query($link,$select);
												while($row=mysqli_fetch_assoc($ret)){
													echo
													"<tr>
														<td align='center'>
														<a href='student_inf2.php?seme=$_GET[seme] & con=$_GET[con] & grade=$_GET[grade] & class=$row[class_name]'>
														$row[class_name]
														</a>
														</td>
