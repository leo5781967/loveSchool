...
if($n==0){
	...
	while($row=mysqli_fetch_assoc($ret1)){
		...
		if(isset($_GET["account"])){
			$time1_hr_sql="SELECT * FROM `reservation` as a inner join `service_hours` as b on a.`res_id`=b.`res_id` inner join `master_list` as c on a.`mas_id`=c.`mas_id` where c.`account_id`='$_GET[account]' and a.`res_id`='$row[res_id]'";
		}else{
			$time1_hr_sql="SELECT * FROM `reservation` as a inner join `service_hours` as b on a.`res_id`=b.`res_id` inner join `master_list` as c on a.`mas_id`=c.`mas_id` where c.`account_id`='$row[account_id]' and a.`res_id`='$row[res_id]'";
		}
		
		$time_hr_sql=mysqli_query($link,$time1_hr_sql);
		$total_hr=$row["res_hr"];
		/* echo "$row[mas_id] &nbsp";
		echo "$row[res_id] &nbsp";
		echo "$total_hr &nbsp";
		echo ""; */
		while($sel3=mysqli_fetch_assoc($time_hr_sql)){
			$total_hr=round($total_hr-$sel3["hr"]);
		...
		if($total_hr == 0){
			if($s%2==0){
				...
				";
					...
					echo "<font style='color:blue;'>已完成</font>";
					echo "</td>";
					echo "</tr>";
			// $row["res_status"]==2 未過期
			// $row["mas_status"]==1 and $row["res_status"]==3 超過期限無法上傳
			// $row["res_status"]==3 超過期限
			// $row["res_status"]==1 已完成
			}else{
				echo "
			...
			if($s%2==0){
				...
				";
					...
					";
					echo "
					<td align='center'><input type='button' class='red' value='刪除' onclick='del($row[res_id]);'></td>
					";
			}
			else{
				echo "
