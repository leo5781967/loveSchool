								...
								<header>
									...
									<?php
											...
											while($row1=mysqli_fetch_assoc($count1)){
												$select="SELECT e.`stu_grade`,count(DISTINCT a.`account_id`) as `people`,sum(b.`res_hr`) as `totalhr` FROM `master_list` as a inner join `reservation` as b on a.`mas_id`=b.`mas_id` inner join `units` as d on b.`unit_id`=d.`unit_id` inner join `student_information` as e on a.`account_id`=e.`account_id` inner join `teacher_information` as f on a.`teacher_id`=f.`teacher_id` where b.`res_status`='1' and a.`content_number`='0' and e.`stu_dep` LIKE '%$tt%' and a.`semester`='$_GET[seme]' and a.`content`='$_GET[con]' and e.`stu_grade`='$row1[stu_grade]'";
												$ret=mysqli_query($link,$select);
												while($row=mysqli_fetch_assoc($ret)){
													echo
													"<tr>
														<td align='center'>
														<a href='t_statistics_3.php?seme=$_GET[seme] & con=$_GET[con] & grade=$row[stu_grade]'>
														$row[stu_grade] 年級
														</a>
														</td>
