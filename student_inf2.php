								...
								<header>
									...
									<?php
											...
											while($row=mysqli_fetch_assoc($count1)){
												$select1="SELECT e.`account_id`,e.`stu_name`,count(a.`account_id`) as `totalpeople`,sum(b.`res_hr`) as `totalhr` FROM `master_list` as a inner join `reservation` as b on a.`mas_id`=b.`mas_id` inner join `units` as d on b.`unit_id`=d.`unit_id` inner join `student_information` as e on a.`account_id`=e.`account_id` inner join `teacher_information` as f on a.`teacher_id`=f.`teacher_id` where b.`res_status`='1' and `content_number`='0' and e.`stu_dep` LIKE '%$tt%' and a.`semester`='$_GET[seme]' and a.`content`='$_GET[con]' and e.`stu_grade`='$_GET[grade]' and e.`class_name`='$_GET[class]' and e.`account_id`='$row[account_id]'";
												...
												while($row1=mysqli_fetch_assoc($ret1)){
													...
													"<tr>
														...
														<td align='center'>
														<a href='student_inf2_1.php?seme=$_GET[seme] & con=$_GET[con] & grade=$_GET[grade] & class=$_GET[class] & name=$row1[stu_name]'>
														$row1[totalhr]
														</a>
														</td>
													</tr>";
