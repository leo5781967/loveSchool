						...
						<!-- Banner -->
							<section id="banner">
								<header>
										...
										<h2 style='color:#4B0091;'><?php echo $_GET["seme"]." 其他 ".$_GET["con"].$_GET["stu"].$_GET["grade"]."年  ".$_GET["class"]."班　".$_GET["name"]; ?></h2>
								</header>
									<?php																		
											$select1="SELECT * FROM `master_list` AS a INNER JOIN `reservation` AS b ON a.`mas_id` = b.`mas_id` INNER JOIN `service_hours` AS c ON b.`res_id` = c.`res_id` INNER JOIN `units` AS d ON b.`unit_id` = d.`unit_id` inner join `student_information` as e on a.`account_id`=e.`account_id` inner join `teacher_information` as f on c.`teacher_id`=f.`teacher_id` where b.`res_status`='1' and a.`content_number` and a.`semester`='$_GET[seme]' and a.`content`='$_GET[con]' and e.`stu_dep`='$_GET[stu]' and e.`stu_grade`='$_GET[grade]' and e.`class_name`='$_GET[class]' and e.`stu_name`='$_GET[name]' GROUP BY b.`res_id`";
