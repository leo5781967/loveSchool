					...
					<div id="page-wrapper">
							<section id="banner">
								<header>
										...
										<h2 style='color:#4B0091;'><?php echo $_GET["seme"].$_GET["stu"].$_GET["grade"]."年  ".$_GET["class"]."班　".$_GET["name"]."同學"; ?></h2>
								</header>
						
									<?php																		
											$select1="SELECT * FROM `master_list` as a inner join `reservation` as b on a.`mas_id`=b.`mas_id` inner join `service_hours` as c on b.`res_id`=c.`res_id` inner join `units` as d on b.`unit_id`=d.`unit_id` inner join `student_information` AS e ON a.`account_id` = e.`account_id` inner join `teacher_information` as f on c.`teacher_id`=f.`teacher_id` where b.`res_status`='1' and a.`semester`='$_GET[seme]' and e.`stu_dep`='$_GET[stu]' and e.`stu_grade`='$_GET[grade]' and e.`class_name`='$_GET[class]' and e.`stu_name`='$_GET[name]' GROUP BY b.`res_id`";
